...
class Transform_Net(nn.Module):
    def __init__(self):
        ...
        self.transform = nn.Linear(256, 6*6)     # (256, 3*3)
        init.constant_(self.transform.weight, 0)
        init.eye_(self.transform.bias.view(6, 6))    # self.transform.bias.view(3, 3)
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, 6, 6)            # (batch_size, 3*3) -> (batch_size, 3, 3)   #x = x.view(batch_size, 3, 3)
